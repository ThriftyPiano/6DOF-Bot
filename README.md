...
### 1. Wide-Angle Lens Distortion Breaking Pose Estimation

**Problem:** The Arducam wide-angle camera introduced severe barrel distortion, making AprilTag corner positions inaccurate. The standard `solvePnP` algorithm produced ~3" localization error because it was fitting a pose to distorted pixel coordinates.

**Solution:** Performed intrinsic camera calibration using a checkerboard pattern to extract a 5-parameter distortion model (k1, k2, p1, p2, k3). Applied `Calib3d.undistortPoints()` to tag corner detections before pose estimation, and full-frame `Calib3d.undistort()` for the artifact detection pipeline. This required careful resolution scaling since the FTC SDK captures at 1920×1080 but calibration was done at 1280×720.

### 2. Reducing Localization Error with Constrained Pose Estimation
...
### 3. Deploying YOLOv8 on Mobile Hardware
...
### 4. Perspective Homography for Sample Inspection
...
### 5. Projecting 2D Detections to Field Coordinates
...
### 6. Analytical 6-DOF Inverse Kinematics with Reach Safety
...
### 7. Adaptive Camera Exposure Control
...
### 8. Desktop Testing Without Robot Hardware
